...
func NewMinMax() opsagent.Aggregator {
	mm := &MinMax{}
	mm.Reset()
	return mm
}
...
func (m *MinMax) Add(in opsagent.Metric) {
	id := in.HashID()
...
func (m *MinMax) Push(acc opsagent.Accumulator) {
...
func init() {
	aggregators.Add("minmax", func() opsagent.Aggregator {
		return NewMinMax()
	})
}
