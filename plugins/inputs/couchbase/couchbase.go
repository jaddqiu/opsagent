...
func (r *Couchbase) Gather(acc opsagent.Accumulator) error {
	if len(r.Servers) == 0 {
...
func (r *Couchbase) gatherServer(addr string, acc opsagent.Accumulator, pool *couchbase.Pool) error {
	if pool == nil {
...
func init() {
	inputs.Add("couchbase", func() opsagent.Input {
		return &Couchbase{}
	})
}
