...
const (
	...
	// DefaultShutdownTimeout is the max amount of time opsagent will wait
...
const sampleConfig = `
  ...
  # port = 9411            # Port on which Opsagent listens
`

// Zipkin is a opsagent configuration structure for the zipkin input plugin,
...
// Description is a necessary method implementation from opsagent.ServiceInput
...
// SampleConfig is a  necessary  method implementation from opsagent.ServiceInput
...
func (z *Zipkin) Gather(acc opsagent.Accumulator) error { return nil }

// Start launches a separate goroutine for collecting zipkin client http requests,
// passing in a opsagent.Accumulator such that data can be collected.
func (z *Zipkin) Start(acc opsagent.Accumulator) error {
...
func (z *Zipkin) Listen(ln net.Listener, acc opsagent.Accumulator) {
	if err := z.server.Serve(ln); err != nil {
		...
		// This interferes with opsagent's internal data collection,
...
func init() {
	inputs.Add("zipkin", func() opsagent.Input {
		return &Zipkin{
			Path: DefaultRoute,
			Port: DefaultPort,
		}
