...
var sampleConfig = `
  ...
  ## Sudo must be configured to to allow the opsagent user to run smartctl
  ## with out password.
  # use_sudo = false
  #
...
func (m *Smart) Gather(acc opsagent.Accumulator) error {
	if len(m.Path) == 0 {
...
func (m *Smart) getAttributes(acc opsagent.Accumulator, devices []string) {
...
func gatherDisk(acc opsagent.Accumulator, usesudo, attributes bool, smartctl, nockeck, device string, wg *sync.WaitGroup) {
...
func init() {
	...
	inputs.Add("smart", func() opsagent.Input {
		return &m
	})
}
